...
class ModifiedAdam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999, 0.9, 0.999), eps=1e-8,
...
def _single_tensor_adam(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if capturable or differentiable:
            ...
            bias_correction1 = 1 - torch.pow(beta3, step)
            bias_correction2 = 1 - torch.pow(beta4, step)
            ...
            bias_correction1 = 1 - beta3 ** step
            bias_correction2 = 1 - beta4 ** step
...
def _multi_tensor_adam(params: List[Tensor],
    ...
    if capturable:
        ...
        bias_correction1 = [torch.pow(beta3, step) for step in state_steps]
        bias_correction2 = [torch.pow(beta4, step) for step in state_steps]
        ...
        bias_correction1 = [1 - beta3 ** step.item() for step in state_steps]
        bias_correction2 = [1 - beta4 ** step.item() for step in state_steps]
